...
public class IcatStore implements IWebdavStore {
	...
	private List<String> icatAuthenticators = null; 
...
//	private static final String CURLY_BRACES = Pattern.quote("{}"); // if using as a regex
    ...
    // to be populated from values in the properties file
    ...
    public IcatStore() throws Exception {
    	try {
        	...
        	String icatUrlString = properties.getProperty("icatURL");
        	// currently assuming that ICAT and IDS are on the same machine
			URL icatAndIdsServerURL = new URL(icatUrlString);
			URL icatUrl = new URL(icatAndIdsServerURL, "/ICATService/ICAT?wsdl");
			LOG.info("icatUrl is: '" + icatUrl + "'");
			ICATService icatService = new ICATService(icatUrl, new QName("http://icatproject.org", "ICATService"));
			icatEP = icatService.getICATPort();
			idsClient = new IdsClient(icatAndIdsServerURL);
			String icatAuthenticatorsString = properties.getProperty("icatAuthenticators");
			// split the property on whitespace
			icatAuthenticators = Arrays.asList(icatAuthenticatorsString.split("\\s+"));
			String sessionRefreshMarginMinsString = properties.getProperty("sessionRefreshMarginMins");
			if (sessionRefreshMarginMinsString == null) {
				String message = "Property 'sessionRefreshMarginMins' is null in properties file";
				LOG.error(message);
			...
			try {
				...
				String message = "Error parsing 'sessionRefreshMarginMins' value of '" + sessionRefreshMarginMinsString + "'";
				LOG.error(message, e);
				throw new Exception(message, e);
			}
		    instrumentName = properties.getProperty("instrumentName");
		    investigationTypeName = properties.getProperty("investigationTypeName");
		    datasetTypeName = properties.getProperty("datasetTypeName");
		    datafileFormatName = properties.getProperty("datafileFormatName");
		    
			LOG.info("icatAuthenticators are: " + icatAuthenticators);
			LOG.info("sessionRefreshMarginMins is: " + sessionRefreshMarginMins);
			LOG.info("instrumentName is: '" + instrumentName + "'");
			LOG.info("investigationTypeName is: '" + investigationTypeName + "'");
			LOG.info("datasetTypeName is: '" + datasetTypeName + "'");
			LOG.info("datafileFormatName is: '" + datafileFormatName + "'");
			...
			LOG.error("Problem loading properties from file: '" + Utils.PROPERTIES_FILENAME + "'", e);
			throw e;
		}
    }
    	...
    	synchronized(sessionMapsLock) {
	    	...
	    	if (sessionId == null || now.after(sessionTimeoutDate)) {
	    		...
	    		// if we are within the specified number of minutes of the session timeout then refresh it
	       		Date sessionRefreshDate = new Date(sessionTimeoutDate.getTime() - (sessionRefreshMarginMins * 60 * 1000));
    ...
    private void doIcatLogin(String authString) throws UnauthenticatedException {
    	...
    	synchronized(sessionMapsLock) {
			...
			for (String icatAuthenticator : icatAuthenticators) {
				try {
    ...
    // KP 24/09/15 - this is the old version of getChildrenNames
    // and might be needed in the future if the code is used on existing
    // ICATs where the datafiles have not been put in using IDAV.
    // Because IDAV creates pseudo folder entries (as Datafiles with 
    // the description FOLDER) this method can be made more efficient
    // so that it does not return any more Datafile records than necessary
    // in order to find the children at any level.
    // Using the old method it is necessary to return all the Datafiles
    // below a certain folder - perhaps many thousands - just to find out
    // that there are a couple of sub-folders in this folder. 
//    public String[] getChildrenNamesOld(String authString, String uri)
...
//    		// this is the root of the server - search for facilities
//    		icatQuery = "SELECT facility.name from Facility facility";
...
//    		// search for investigations in this facility
//    		icatQuery = "SELECT investigation.title from Investigation investigation" + createWhereClause(icatEntityNames);
//    	} else if (uriParts.length == 3) {
//    		// search for datasets in this investigation and facility
//    		icatQuery = "SELECT dataset.name from Dataset dataset" + createWhereClause(icatEntityNames);
...
//    	} else {
//    		// re-combine all of the parts beyond 4
//    		// and add datafile.name like 'datafilepath%' to the query
//    		// search for datafiles in this dataset, investigation and facility
//    		icatQuery = "SELECT datafile.name from Datafile datafile" + createWhereClause(icatEntityNames, DatafileSearchType.LIKE);
//    	}
//
...
//			// Facility, Investigation and Dataset listings are straightforward
...
//    		throw new WebdavException(message);
    ...
    private DatafileFormat getDatafileFormat(String authString) throws WebdavException {
		if (datafileFormat == null) {
			...
			String icatQuery = "SELECT datafileFormat from DatafileFormat datafileFormat WHERE datafileFormat.name='" + datafileFormatName + "'";
			LOG.debug("icatQuery = [" + icatQuery + "]");
    ...
    private DatasetType getDatasetType(String authString) throws WebdavException {
		if (datasetType == null) {
			...
			String icatQuery = "SELECT dsType from DatasetType dsType WHERE dsType.name='" + datasetTypeName + "'";
			LOG.debug("icatQuery = [" + icatQuery + "]");
    ...
    private InvestigationType getInvestigationType(String authString) throws WebdavException {
		if (investigationType == null) {
			...
			String icatQuery = "SELECT invType from InvestigationType invType WHERE invType.name='" + investigationTypeName + "'";
			LOG.debug("icatQuery = [" + icatQuery + "]");
    ...
    private Instrument getInstrument(String authString) throws WebdavException {
		if (instrument == null) {
			...
			String icatQuery = "SELECT inst from Instrument inst WHERE inst.name='" + instrumentName + "'";
			LOG.debug("icatQuery = [" + icatQuery + "]");
